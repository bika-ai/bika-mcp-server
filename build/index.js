...
server.tool("list_spaces", "Fetches all workspaces that the currently authenticated user has permission to access.", async () => {
    try {
        const result = await bikaService.fetchFromAPI("/v1/spaces", {
            method: "GET",
        });
        if (!result.success) {
        ...
        const spaces = result.data.map(space => ({
            space_id: space.id,
...
server.tool("list_nodes", "Retrieves all nodes contained within the specified workspace. Nodes in Bika can be of several types: databases (also known as sheets, datasheets, or spreadsheets), automations, documents, and folders.", {
    ...
    try {
        ...
        const result = await bikaService.fetchFromAPI(`/v1/spaces/${space_id}/nodes`, {
            method: "GET",
        });
        if (!result.success) {
...
server.tool("get_records", "Read the records from a specified database with support for pagination, field filtering, and sorting options.", {
    ...
    try {
        ...
        const result = await bikaService.fetchFromAPI(endpoint, {
            method: "GET",
        });
        if (!result.success) {
...
server.tool("get_fields_schema", "Returns the JSON schema of all fields within the specified database, This schema will be sent to LLM to help the AI understand the expected structure of the data.", {
    space_id: z.string().describe('The ID of the workspace to fetch records from.'),
    node_id: z.string().describe('The ID of the database to fetch records from.'),
    ...
    try {
        ...
        const fieldsSchema = bikaService.getFieldsJSONSchema(result.data);
        return formatToolResponse({
            success: true,
            data: fieldsSchema
        });
...
server.tool("create_record", "Create a new record in the database. Extract key information from user-provided text based on a predefined Fields JSON Schema and create a new record in the database as a JSON object.", {
...
server.tool("upload_attachment_via_url", "Upload an attachment to the Bika server using its web URL. Returns storage information that can be passed to create_record or update_record tools to associate with a specific records.", {
    ...
    try {
        const result = await bikaService.uploadFileToSpace(space_id, attachment_url, attachment_name);
        if (!result.success) {
            return formatToolResponse({
                success: false,
