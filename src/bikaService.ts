...
export class BikaService {
  ...
  constructor(
    apiKey: string = process.env.BIKA_API_KEY ?? "",
    baseUrl: string = "https://bika.ai/api/openapi/bika",
  ...
  public async fetchFromAPI<T>(
    ...
    if (!response.ok) {
      throw new Error(
        `Bika API Error: ${response.statusText}. Response: ${responseJson.message}`
      );
    }
  ...
  private _getKeywordByFieldType(field: FieldSchemaVO): object | null {
    if (
      ["LONG_TEXT", "SINGLE_TEXT", "EMAIL", "URL", "PHONE"].includes(field.type)
    ) {
      return { type: "string" };
    }
  ...
  public convertFieldValuesToCellFormat(
    ...
    fieldsSchema.forEach((fieldschema) => {
      ...
      if (fieldValue !== undefined) {
        cells[fieldschema.name] = this._getCellValueByFieldType(
          fieldschema,
          fieldValue
        );
      }
  ...
  public async getDatabaseFieldsSchema(
    space_id: string,
    node_id: string
  ): Promise<ResponseVO<FieldSchemaVO[]>> {
    if (!space_id || !node_id) {
  ...
  public async createDatabaseRecord(
    space_id: string,
    node_id: string,
    cells: Record<string, unknown>
  ): Promise<ResponseVO<RecordV2VO>> {
    if (!space_id || !node_id) {
      throw new Error("space_id and node_id are required.");
    }
